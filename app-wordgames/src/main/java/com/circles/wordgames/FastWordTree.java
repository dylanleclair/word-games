...
public class FastWordTree extends WordTree {
    ...
    private void findPermutationsHelper (FastNode node, ArrayList<Character> characters, ArrayList<String> permutations) throws Exception { 
        ...
        if (characters.size() > 0 ) {
            for (char c : characters) {
                ...
                if (hasChild) {
                    FastNode check = node.getChild(c);
    ...
    public String BubbleUp(FastNode node) {
        ArrayList<Character> chars = new ArrayList<Character>();
